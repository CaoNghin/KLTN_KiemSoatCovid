...
namespace QuanLyCovidDHH.Web.Controllers
{
    public class BenhAnController : Controller
    {
        ...
        public BenhAnController(IWebHostEnvironment webHostEnvironment)
        {
        ...
        public JsonResult DanhSach( Guid? IDSinhVien,Guid? IDKhoaHoc, Guid? IDKhoa, Guid? IDTruong, string TuNgay, string DenNgay, string TrangThaiDieuTri, string TrangThaiDuyet)
        {
            var danhsach = new List<DanhSach>();

            string url = $"{Common.Common.ApiUrl}/BenhAn?IDSinhVien={IDSinhVien}&IDKhoaHoc={IDKhoaHoc}&IDKhoa={IDKhoa}&IDTruong={IDTruong}&TuNgay={TuNgay}&DenNgay={DenNgay}&TrangThaiDieuTri={TrangThaiDieuTri}&TrangThaiDuyet={TrangThaiDuyet}";
        ...
        public JsonResult TaoMoi(TaoMoi model)
        {
            ...
            string path = Path.Combine(_webHostEnvironment.WebRootPath, "files\\BenhAn");
            string filePath1 = null;
            model.BenhAnDinhKem = "";
            var thumuc = "";
            if (model.FileBenhAnDinhKem != null)
            {
                filePath1 = Path.Combine(path, thumuc + model.FileBenhAnDinhKem.FileName);

                model.BenhAnDinhKem = thumuc + model.FileBenhAnDinhKem.FileName;
            }
            ...
            try
            {
                string url = $"{Common.Common.ApiUrl}/BenhAn";
                HttpWebRequest httpWebRequest = (HttpWebRequest)WebRequest.Create(url);
                ...
                if (model.FileBenhAnDinhKem != null)
                {
                    ...
                    {
                        model.FileBenhAnDinhKem.CopyTo(fileStream);
                    }
                }
        ...
        public JsonResult TaoMoi_Excel(TaoMoi model)
        {
            ...
            try
            {
                string url = $"{Common.Common.ApiUrl}/BenhAn/Excel";
                HttpWebRequest httpWebRequest = (HttpWebRequest)WebRequest.Create(url);
                httpWebRequest.ContentType = "application/json";
                httpWebRequest.Method = "POST";
                using (var streamWrite = new StreamWriter(httpWebRequest.GetRequestStream()))
        ...
        {
            var sv = new ChinhSua();
            string url = $"{Common.Common.ApiUrl}/BenhAn/{ID}";
            HttpWebRequest httpWebRequest = (HttpWebRequest)WebRequest.Create(url);
            httpWebRequest.Method = "GET";
            var httpWebResponse = httpWebRequest.GetResponse();
            {
            ...
            return Json(new { code = 200, msg = "Lấy ID thành công", benhan = sv });
        }
