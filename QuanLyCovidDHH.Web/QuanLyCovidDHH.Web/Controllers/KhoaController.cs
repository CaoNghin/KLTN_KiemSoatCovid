...
using QuanLyCovidDHH.Web.Models.Khoa;
using System;
...
namespace QuanLyCovidDHH.Web.Controllers
{
    public class KhoaController : Controller
    {
        ...
        public JsonResult DanhSach(Guid? IDTruong,string TuKhoa )
        {
            var danhsach = new List<DanhSach>();

            string url = $"{Common.Common.ApiUrl}/Khoa?IDTruong={IDTruong}&TuKhoa={TuKhoa}";
            ...
            return Json(new { code = 200, msg = "Success", khoa = danhsach });
        }
        ...
        public JsonResult TaoMoi(TaoMoi model)
        {
            ...
            try
            {
                string url = $"{Common.Common.ApiUrl}/Khoa";
                HttpWebRequest httpWebRequest = (HttpWebRequest)WebRequest.Create(url);
        ...
        {
            var k = new ChinhSua();
            string url = $"{Common.Common.ApiUrl}/Khoa/{ID}";
            HttpWebRequest httpWebRequest = (HttpWebRequest)WebRequest.Create(url);
            httpWebRequest.Method = "GET";
            var httpWebResponse = httpWebRequest.GetResponse();
            {
            ...
            return Json(new { code = 200, msg = "Lấy ID thành công", khoa = k });
        }
